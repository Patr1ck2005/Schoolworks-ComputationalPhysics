...
def q(u):
    ...
    d = 0.00006
    w = w1 + w2 + d
    q = np.logical_or(u >= (w/2-w1), u <= (w2-w/2))
    return q.astype(float)


# 定义被积函数f(u,x),其中x为参数，u为被积变量
def f(u, x):
    return cmath.sqrt(q(u)) * cmath.exp(2j * cmath.pi * u * x / 0.0000005)
...
u = np.linspace(-0.000045, 0.000045, 49)
...
plt.plot(x, y)
plt.savefig(r'C:\Users\26918\Desktop\problem4_e(ii)_1.eps')
...
plt.show()
plt.savefig(r'C:\Users\26918\Desktop\problem4_e(ii)_2.eps')
